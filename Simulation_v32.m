...
figure();
plot(abs(BETAt_k(4,:)));
legend('Betat_k for subcarrier 4');
...
for n = 1:N
    for t = 1:T
        ...
        for i = 1:K
            sumBhr = sumBhr + BETAt_k(t,i) * hr(n,i);
        end

        y(n,t) =  hd(n,1) + sumBhr + w(n,t);
    end
end
...
%This code pertains to first algorithm, please ignore.
pestimation = zeros(N,3);
x0 = p;

% for n = 1:N
%   p_ = fminsearch(@(p_)functionslibv7.objective(p_,n,K,T,y,gdp,G_R,f_n,p_k,BETAt_k,sig2,phi_0,t_0),x0);
%   pestimation(n,:) = p_(1,:);
% end
%
%
